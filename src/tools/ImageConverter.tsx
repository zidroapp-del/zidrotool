...
export default function ImageConverter() {
  ...
  const ext = FORMATS.find((f) => f.value === targetFmt)?.ext ?? "png";
  ...
  return (
    <div>
      ...
      {src && (
        <div className="mt-6">
          <div className="mb-4">
            <label className="label mb-2 block">Convert to</label>
            <div className="flex gap-1.5">
              {FORMATS.map((f) => (
                <button key={f.value} onClick={() => setTargetFmt(f.value)} className={cn("btn btn-sm", targetFmt === f.value ? "btn-primary" : "btn-secondary")}>
                  {f.label}
                </button>
              ))}
            </div>
          ...
          <button onClick={handleConvert} className="btn-primary w-full">Convert Image</button>
        </div>
      )}

      {resultUrl && (
        <div className="mt-6 rounded-xl border border-ink-200 p-4 dark:border-ink-700">
          <p className="mb-2 text-sm font-semibold text-ink-900 dark:text-ink-100">Converted to {FORMATS.find((f) => f.value === targetFmt)?.label}</p>
          <img src={resultUrl} alt="Converted" className="max-h-48 rounded-lg" />
          <a href={resultUrl} download={`converted.${ext}`} className="btn-primary mt-3 inline-flex">
