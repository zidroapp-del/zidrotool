      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-12 lg:grid-cols-12">
        ...
        <div className="grid grid-cols-2 gap-8 sm:grid-cols-4 lg:col-span-8">
          ...
          <FooterCol title={t("footer.legal")}>
            ...
            <button type="button" onClick={openCookieSettings} className="block text-sm text-ink-500 transition-colors hover:text-orange-600 dark:text-ink-400 dark:hover:text-orange-300">{t("cookie.manage")}</button>
          </FooterCol>
        </div>
      </div>
        ...
        {CATEGORIES.map((cat) => {
          ...
          return (
            <Link
              ...
              className="inline-flex items-center gap-1.5 rounded-full border border-ink-200 px-3 py-1 text-xs text-ink-500 transition-colors hover:border-orange-300 hover:text-orange-600 dark:border-ink-700 dark:text-ink-400 dark:hover:border-orange-700 dark:hover:text-orange-300"
            >
          ...
          {socials.map((s) => {
            ...
            return (
              <a
                ...
                className="rounded-lg p-2 text-ink-400 transition-all duration-200 hover:scale-110 hover:bg-ink-100 hover:text-orange-600 dark:hover:bg-ink-800 dark:hover:text-orange-300"
                aria-label={s.label}
              >
...
function FooterLink({
  ...
  return (
    <Link
      to={to}
      className="block text-sm text-ink-500 transition-colors hover:text-orange-600 dark:text-ink-400 dark:hover:text-orange-300"
    >
      {children}
    </Link>
  );
