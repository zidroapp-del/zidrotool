...
export function Navbar({ onOpenSearch }: NavbarProps) {
  ...
  return (
    <header
      className={cn("zt-navbar",
        "sticky top-0 z-50 w-full transition-all duration-300",
        scrolled
          ? "border-b border-ink-200/70 bg-white/75 shadow-sm shadow-brand-900/5 backdrop-blur-xl dark:border-ink-800/70 dark:bg-ink-950/75"
    ...
    >
      <nav className="container-page flex h-[68px] items-center justify-between gap-4" aria-label="Main">
        {/* Logo */}
        <Link to="/" className="group flex shrink-0 items-center gap-2" aria-label="ZidroTool home">
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-orange-500 via-orange-600 to-amber-500 text-white shadow-sm shadow-orange-500/25 transition-transform duration-300 group-hover:rotate-6 group-hover:scale-105">
            <Zap className="h-5 w-5" fill="currentColor" />
          </div>
          <span className="text-lg font-bold tracking-tight text-ink-900 dark:text-ink-100">
            Zidro<span className="text-orange-600 dark:text-orange-300">Tool</span>
          </span>
        </Link>
        ...
        <button
          ...
          className="zt-navbar-search hidden items-center gap-2 rounded-full px-3 py-2 text-sm font-semibold lg:flex"
          aria-label={t("nav.search")}
        >
          <Search className="h-4 w-4" />
          <span className="hidden xl:inline">{t("nav.search")}</span>
          <kbd className="rounded-md px-1.5 py-0.5 font-mono text-[9px]">⌘K</kbd>
        </button>
        ...
        <div className="hidden items-center gap-1 lg:flex">
          ...
          {user ? (
            ...
            <>
              ...
              <Link to="/pricing" className="btn-primary btn-sm zt-navbar-premium">
