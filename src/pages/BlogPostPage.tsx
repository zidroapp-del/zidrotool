...
export default function BlogPostPage() {
  ...
  const [cmsPost, setCmsPost] = useState<any>(null);
  const post: any = cmsPost || (slug ? getPostBySlug(slug) : undefined);
  ...
  const seoDescription = post.seoDesc || excerpt || title;
  const articleWordCount = body.split(/\s+/).filter(Boolean).length;
  ...
  const articleJsonLd = {
    ...
    ...(post.dateModified ? { dateModified: post.dateModified } : post.date ? { dateModified: post.date } : {}),
    author: {
      "@type": "Organization",
      name: author?.name || "ZidroTool Team",
      url: "https://zidrotool.com/blog/author/zidro-team",
    },
    isPartOf: {
      "@type": "Blog",
      name: "ZidroTool Blog",
      url: "https://zidrotool.com/blog",
    },
    publisher: {
      "@type": "Organization",
      name: "ZidroTool",
      ...
      logo: {
        "@type": "ImageObject",
        url: "https://zidrotool.com/icons/icon-512.png",
      },
    },
    mainEntityOfPage: `https://zidrotool.com/blog/${post.slug}`,
    image: `https://zidrotool.com${post.coverImage || getBlogCoverImage(post.slug)}`,
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Blog", item: "https://zidrotool.com/blog" },
      { "@type": "ListItem", position: 3, name: title, item: `https://zidrotool.com/blog/${post.slug}` },
    ],
  };
  ...
  return (
    ...
    <article className="container-page py-12 max-w-4xl mx-auto px-4" dir={isRTL ? "rtl" : "ltr"}>
      ...
      <div className="w-full h-64 md:h-96 rounded-2xl overflow-hidden mb-10 bg-gray-100 dark:bg-gray-800">
        <img
          src={post.coverImage || getBlogCoverImage(post.slug)}
          alt={title}
          loading="eager"
          decoding="async"
          width={1200}
          ...
          onError={(e) => { e.currentTarget.src = getBlogCoverImage(post.slug); }}
        />
      </div>
      ...
      <div className="mb-12 flex flex-col items-center gap-3 border-y border-gray-100 py-6 dark:border-gray-800">
        ...
        <div className="flex items-center gap-2">
          ...
          <a href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(window.location.href)}&text=${encodeURIComponent(title)}`} target="_blank" rel="noopener noreferrer" aria-label="Share on X" className="flex h-9 w-9 items-center justify-center rounded-lg bg-black text-white hover:opacity-90"><Twitter className="h-4 w-4" /></a>
          <a href={`https://t.me/share/url?url=${encodeURIComponent(window.location.href)}&text=${encodeURIComponent(title)}`} target="_blank" rel="noopener noreferrer" aria-label="Share on Telegram" className="flex h-9 w-9 items-center justify-center rounded-lg bg-sky-500 text-white hover:opacity-90"><Send className="h-4 w-4" /></a>
          ...
          <button type="button" onClick={() => navigator.clipboard?.writeText(window.location.href)} aria-label="Copy article link" className="flex h-9 w-9 items-center justify-center rounded-lg bg-gray-700 text-white hover:opacity-90"><LinkIcon className="h-4 w-4" /></button>
        </div>
      </div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {relatedPosts.map((related: any) => {
              ...
              return (
                ...
                >
                  <div>
                    <div className="mb-4 h-40 overflow-hidden rounded-xl bg-gray-100 dark:bg-gray-800">
                      <img src={related.coverImage || getBlogCoverImage(related.slug)} alt={relatedTitle} loading="lazy" className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-105" onError={(e) => { e.currentTarget.src = getBlogCoverImage(related.slug); }} />
                    </div>
                    {related.category && (
